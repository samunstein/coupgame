...
class Methods:
    ...
    def always_captain(self: MockLogic):
        return StealDecision(list(self.get_state().alive_opponents().keys())[0])


class CaptainTest(TestCase, Methods):
    def test_basic_no_blocks(self):
        ...
        self.assertEqual(game.rule_abiding_players[1].money, 0)
    ...
    def test_no_money(self):
        ...
        game = Game(clients, deck=[Captain()] * 4)
        game.setup_players()
        game.rule_abiding_players[1].give_money(-2)
    ...
    def test_unsuccessful_block(self):
        ...
        game = Game(clients, deck=[Captain()] * 4)
        game.setup_players()
        game.rule_abiding_players[1].remove_card(Captain())
        game.rule_abiding_players[1].remove_card(Captain())
        game.rule_abiding_players[1].give_card(Duke())
        game.rule_abiding_players[1].give_card(Duke())
    ...
    def test_ambassador_block(self):
        ...
        game.rule_abiding_players[1].remove_card(Captain())
        game.rule_abiding_players[1].remove_card(Captain())
        game.rule_abiding_players[1].give_card(Ambassador())
        game.rule_abiding_players[1].give_card(Ambassador())

...
class PlayerClient:
    ...
    def run_command(self, command, params) -> object | None:
        # Setup and meta
        ...
        # Turn flow
            ...
            action, taken_by, block_card, challenger = all_actions_map()[params[0]], int(params[1]), params[2], int(
                params[3])
            ...
            action, taken_by, target, block_card, blocker = all_actions_map()[params[0]], int(params[1]), int(
                params[2]), params[3], int(params[4])
        ...
        # Log
            ...
            action, taken_by, target, block_card, blocker, challenger, success = (
                all_actions_map()[params[0]], int(params[1]), int(params[2]), params[3], int(params[4]), int(params[5]),
                params[6] == "True"
            )
    ...
    def run(self):
        while self.running:
            ...
            for command_data in commands:
                ...
                response = self.run_command(command, params)
